...
export interface LegacyLiveTickServiceDelegates {
  combatLiveTick?: () => void;
...
const legacyLiveTickServiceDelegates: LegacyLiveTickServiceDelegates = {};

export function registerLegacyLiveTickServiceDelegates(delegates: LegacyLiveTickServiceDelegates): void {
  if (delegates.combatLiveTick) {
    legacyLiveTickServiceDelegates.combatLiveTick = delegates.combatLiveTick;
  }
  if (delegates.objectiveFastLiveTick) {
    legacyLiveTickServiceDelegates.objectiveFastLiveTick = delegates.objectiveFastLiveTick;
  }
  if (delegates.objectiveSlowLiveTick) {
    legacyLiveTickServiceDelegates.objectiveSlowLiveTick = delegates.objectiveSlowLiveTick;
  }
  if (delegates.spawnRoutingLiveTick) {
    legacyLiveTickServiceDelegates.spawnRoutingLiveTick = delegates.spawnRoutingLiveTick;
  }
}
...
function Mode_OngoingGlobal_Inner(): void {
  ...
  if (gameStatus === 0) {
    ...
    if (legacyLiveTickServiceDelegates.combatLiveTick) {
      legacyLiveTickServiceDelegates.combatLiveTick();
    } else {
      runLegacyCombatLiveTick();
    }

    if (mod.Modulo(phaseTickCount, LIVE_FAST_UPDATE_INTERVAL_TICKS) === 0) {
      if (legacyLiveTickServiceDelegates.objectiveFastLiveTick) {
        legacyLiveTickServiceDelegates.objectiveFastLiveTick();
      } else {
        runLegacyObjectiveFastLiveTick();
      }

      if (legacyLiveTickServiceDelegates.spawnRoutingLiveTick) {
        legacyLiveTickServiceDelegates.spawnRoutingLiveTick();
      } else {
    ...
    if (mod.Modulo(phaseTickCount, LIVE_SLOW_UPDATE_INTERVAL_TICKS) === 0) {
      if (legacyLiveTickServiceDelegates.objectiveSlowLiveTick) {
        legacyLiveTickServiceDelegates.objectiveSlowLiveTick();
      } else {
...
export const LegacyCombatBridge = {
...
export const LegacyObjectiveBridge = {
...
export const LegacySpawnRoutingBridge = {
...
export const DominationModeHandlers = {
