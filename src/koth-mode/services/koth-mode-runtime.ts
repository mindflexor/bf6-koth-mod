...
class KothPhaseRuntimeFacade {
    ...
    private readonly _bombService = new BombService(this._context);
    ...
    public setSpectatorController(controller?: OptionalObserverController): void {
        setKernelSpectatorController(controller);
        configureKothLiveRuntime(controller);
    }

    public canEnterSpectatorMode(eventPlayer: mod.Player): boolean {
        return runtimeCanEnterKothLiveSpectatorMode(eventPlayer) || kernelCanEnterKothSpectatorMode(eventPlayer);
    }

    public resolveSpectatorSpawnPoint(eventPlayer: mod.Player, teamId: number): number {
        const kothSpawnPoint = runtimeResolveKothLiveSpectatorSpawnPoint(eventPlayer, teamId);
        return kothSpawnPoint || kernelResolveKothSpectatorSpawnPoint(eventPlayer, teamId);
    }
...
export function configureKothModeRuntime(spectatorController?: OptionalObserverController): void {
    phaseFacade.setSpectatorController(spectatorController);
}

export function runtimeCanEnterKothPhaseSpectatorMode(eventPlayer: mod.Player): boolean {
    return phaseFacade.canEnterSpectatorMode(eventPlayer);
}

export function runtimeResolveKothPhaseSpectatorSpawnPoint(eventPlayer: mod.Player, teamId: number): number {
    return phaseFacade.resolveSpectatorSpawnPoint(eventPlayer, teamId);
}
