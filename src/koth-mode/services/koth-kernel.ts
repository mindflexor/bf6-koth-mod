/* =================================================================================================
   Mind Flexor - King Of The Hill Game Mode Script
   -------------------------------------------------------------------------------------------------
   

   Description:
     - Preserves the original prematch, ready-up, prelive, and postmatch phase runtime:
...
let kernelSpectatorController: OptionalObserverController | undefined;
let kernelKothLiveOverrideEnabled = false;
const SPECTATOR_SPAWN_POINT_ID = WORLD_IDS.spectator.spawnPoint;

export function setKernelSpectatorController(controller?: OptionalObserverController): void {
  kernelSpectatorController = controller;
}
...
export function kernelCanEnterKothSpectatorMode(_eventPlayer: mod.Player): boolean {
  return gameStatus === 0;
}

export function kernelResolveKothSpectatorSpawnPoint(_eventPlayer: mod.Player, _teamId: number): number {
  if (gameStatus < 0 || gameStatus > 3) return 0;
  return resolveSpectatorRecoverySpawnPoint();
}


function isSpectatorPlayer(player: mod.Player): boolean {
  return kernelSpectatorController?.isSpectator(player) ?? false;
}
...
let spectatorSpawnIsolationValidated: boolean = false;
...
function isSpectatorSpawnPointId(spawnerObjId: number): boolean {
  return SPECTATOR_SPAWN_POINT_ID > 0 && spawnerObjId === SPECTATOR_SPAWN_POINT_ID;
}

function filterSpectatorSpawnPointIds(spawnerObjIds: number[] | undefined): number[] {
  if (!spawnerObjIds || spawnerObjIds.length <= 0) return [];
  return spawnerObjIds.filter((spawnerObjId) => !isSpectatorSpawnPointId(spawnerObjId));
}

function sanitizeParticipantSpawnerObjId(spawnerObjId: number): number {
  if (!spawnerObjId || spawnerObjId <= 0) return 0;
  return isSpectatorSpawnPointId(spawnerObjId) ? 0 : spawnerObjId;
}

function resolveSpectatorRecoverySpawnPoint(): number {
...
function getSpawnersForTeamAndRoute(team: mod.Team, routeKey: DynamicRouteKey): number[] {
  ...
  return filterSpectatorSpawnPointIds(configured);
}
...
function isSpectatorReservedLiveHudSlot(teamKey: LiveUiTeamKey, slotIndex: number): boolean {
  return teamKey === SPECTATOR_LIVE_UI_TEAM_KEY && slotIndex === SPECTATOR_LIVE_UI_SLOT_INDEX;
}

function getSpectatorLiveHudSlotAssignment(): LiveUiSlotAssignment {
  return {
...
function allocateLiveHudSlotForPlayer(p: Player): LiveUiSlotAssignment | undefined {
  ...
  for (let i = 0; i < LIVE_UI_SLOTS_PER_TEAM; i++) {
    if (isSpectatorReservedLiveHudSlot(teamKey, i)) continue;
...
function ensureSpectatorLiveHudSlotForPlayer(p: Player): boolean {
  const assignment = getSpectatorLiveHudSlotAssignment();
...
function ensureLiveHudSlotForPlayer(p: Player, allowHiddenBuild: boolean = false): boolean {
  ...
  const spectator = isSpectatorPlayer(p.player);
  ...
  if (spectator) {
    ...
    return ensureSpectatorLiveHudSlotForPlayer(p);
  }
...
function getLiveHudPerspectiveTeamForPlayer(p: Player): mod.Team {
  return isSpectatorPlayer(p.player) ? team1 : mod.GetTeam(p.player);
}
...
class Player {
  ...
  addUI(): void {
    ...
    const spectator = isSpectatorPlayer(this.player);
  ...
  updateTickets(): void {
    ...
    if (spectator) {
      if (!ensureSpectatorLiveHudSlotForPlayer(this)) return;
      setLiveHudVisibleForPlayer(this, true);
    } else {
    ...
    const scorePerspectiveTeam = spectator ? team1 : mod.GetTeam(this.player);
...
function applyPrematch889HealthForPlayer(playerId: number): void {
  ...
  if (!sp) return;
  if (!mod.IsPlayerValid(sp.player)) return;
  if (isSpectatorPlayer(sp.player)) return;
...
function applyPrematch889HealthForAllPlayers(): void {
  serverPlayers.forEach((p) => {
    if (isSpectatorPlayer(p.player)) return;
...
function validateSpectatorSpawnIsolationFromConfigOnce(): void {
  if (spectatorSpawnIsolationValidated) return;
  spectatorSpawnIsolationValidated = true;

  if (SPECTATOR_SPAWN_POINT_ID <= 0) return;
  ...
  for (let i = 0; i < hqEntries.length; i++) {
    if (hqEntries[i][1] === SPECTATOR_SPAWN_POINT_ID) {
  ...
  for (let i = 0; i < routeKeys.length; i++) {
    ...
    if (team1RouteSpawners.indexOf(SPECTATOR_SPAWN_POINT_ID) >= 0) {
      collisions.push("dynamic.team1." + routeKey);
    }

    if (team2RouteSpawners.indexOf(SPECTATOR_SPAWN_POINT_ID) >= 0) {
  ...
  LogRuntimeError(
    "SpectatorSpawnIsolation",
    "spectator spawn id " + String(SPECTATOR_SPAWN_POINT_ID) + " collides with: " + collisionSummary
  );
  mod.DisplayHighlightedWorldLogMessage(
    mod.Message("[SPECTATOR SPAWN] id {} collides with {}", SPECTATOR_SPAWN_POINT_ID, collisionSummary)
  );
}
...
function InitializePreMatch(): void {
  ...
  validateSpectatorSpawnIsolationFromConfigOnce();
...
function Mode_OngoingGlobal_Inner(): void {
  ...
  if (gameStatus === 0) {
    ...
    let humanSpectatorPlayers = 0;

  serverPlayers.forEach((p) => {
      p.setTeam();
      if (isBotBackfillPlayer(p.player)) return;
      if (isSpectatorPlayer(p.player)) {
        humanSpectatorPlayers += 1;
        return;
      }
    ...
    const allReady =
      ...
      (humanParticipantPlayers > 0 || humanSpectatorPlayers > 0);
...
function Mode_OnPlayerJoinGame(eventPlayer: mod.Player): void {
  ...
  if (player && !isSpectatorPlayer(player.player)) {
  ...
  if (isSpectatorPlayer(eventPlayer)) {
...
function Mode_OnPlayerLeaveGame(eventNumber: number): void {
  ...
  if (!leaving) return;
  const leavingWasSpectator = isSpectatorPlayer(leaving.player);
  ...
  if (gameStatus === 3 && !leavingWasSpectator) {
    leaving.addDeath();
...
async function Mode_OnPlayerDeployed(eventPlayer: mod.Player): Promise<void> {
  ...
  if (isSpectatorPlayer(eventPlayer) || (!mod.Equals(team, team1) && !mod.Equals(team, team2))) {
    return;
  }
...
function Mode_OnPlayerInteract(eventPlayer: mod.Player, eventInteractPoint: mod.InteractPoint): void {
  if (isSpectatorPlayer(eventPlayer)) return;
...
function Mode_OnPlayerDamaged(
  ...
  if (!mod.IsPlayerValid(eventPlayer)) return;
  if (!isParticipantPlayer(eventPlayer)) return;
  if (mod.IsPlayerValid(eventOtherPlayer) && isSpectatorPlayer(eventOtherPlayer)) return;
