...
import { createKothModeContext } from '../state/koth-mode-context.ts';
...
class KothRuntimeFacade {
    private readonly _context = createKothModeContext();
...
const facade = new KothRuntimeFacade();

export function configureKothRuntime(spectatorController?: OptionalObserverController): void {
    facade.setSpectatorController(spectatorController);
}

export function runtimeCanEnterKothSpectatorMode(eventPlayer: mod.Player): boolean {
    return facade.canEnterSpectatorMode(eventPlayer);
}

export function runtimeResolveKothSpectatorSpawnPoint(eventPlayer: mod.Player, teamId: number): number {
    return facade.resolveSpectatorSpawnPoint(eventPlayer, teamId);
}

export const KothModeHandlers = {
    OnGameModeStarted: (): void => facade.onGameModeStarted(),
    OnGameModeEnding: (): void => facade.onGameModeEnding(),
    OnPlayerJoinGame: (eventPlayer: mod.Player): void => facade.onPlayerJoinGame(eventPlayer),
    OnPlayerLeaveGame: (eventNumber: number): void => facade.onPlayerLeaveGame(eventNumber),
    OnPlayerDeployed: (eventPlayer: mod.Player): void => facade.onPlayerDeployed(eventPlayer),
    OnPlayerUndeploy: (eventPlayer: mod.Player): void => facade.onPlayerUndeploy(eventPlayer),
    OnPlayerDied: (eventPlayer: mod.Player): void => facade.onPlayerDied(eventPlayer),
    OnMandown: (eventPlayer: mod.Player): void => facade.onMandown(eventPlayer),
    OnPlayerEarnedKill: (eventPlayer: mod.Player, eventOtherPlayer: mod.Player): void =>
        facade.onPlayerEarnedKill(eventPlayer, eventOtherPlayer),
    OnPlayerEarnedKillAssist: (eventPlayer: mod.Player): void => facade.onPlayerEarnedKillAssist(eventPlayer),
